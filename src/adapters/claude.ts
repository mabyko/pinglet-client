...
/** 슬래시 명령이 호출하는 CLI 서브커맨드 — 권한 규칙과 command 파일 모두 이 목록 기준. */
const SLASH_SUBCOMMANDS = ["post", "login", "logout", "hud"] as const;
const COMMAND_FILES = ["pinglet.md", "pinglet-login.md", "pinglet-logout.md", "pinglet-hud.md"];
...
/**
 * `/pinglet`, `/pinglet-login`, `/pinglet-logout` 인라인 bash를 허용하는 settings.json 권한 규칙.
 * auto 권한 모드에서는 allow 규칙에 없는 Bash 명령을 LLM 분류기가 심사하는데,
 * 메시지 내용("~해라" 같은 명령문)에 따라 차단될 수 있어 규칙으로 명시한다.
 */
function slashPermissionRules(): string[] {
  return SLASH_SUBCOMMANDS.map((sub) => `Bash(node "${cliPath()}" ${sub}:*)`);
}

/** Pinglet이 등록한 권한 규칙인지 식별 (경로가 바뀐 옛 규칙 포함). */
function isPingletPermissionRule(rule: string): boolean {
  return (
    rule.startsWith('Bash(node "') &&
    rule.includes("pinglet") &&
    SLASH_SUBCOMMANDS.some((sub) => rule.endsWith(`" ${sub}:*)`))
  );
}

/**
 * `/pinglet`, `/pinglet-login`, `/pinglet-logout` slash command 파일을 설치한다.
 * 실제 동작은 command 안의 인라인 bash(`!` 실행)가 수행하고,
 * 모델은 그 결과만 전달하므로 토큰 사용이 매우 적다.
...
`,
    "pinglet-hud.md": `---
description: ${t("slash.hudDesc")}
allowed-tools: Bash(node:*)
---
<!-- ${COMMAND_MARKER} -->

Result: !\`node "${cli}" hud --quiet $ARGUMENTS\`
...
`,
  ...
  for (const [name, body] of Object.entries(files)) {
    fs.writeFileSync(path.join(COMMANDS_DIR, name), body);
  }
}

/**
 * 누락된 slash command 파일만 채운다 — 자동 업데이트가 postinstall 없이 끝났을 때
 * 새 명령(`/pinglet-hud` 등)이 생기도록 refresh가 호출한다. 있는 파일은 건드리지 않는다.
 */
export function ensureSlashCommands(config: PingletConfig): void {
  if (!config.adapters.claude) return;
  const missing = COMMAND_FILES.some((name) => !fs.existsSync(path.join(COMMANDS_DIR, name)));
  if (missing) installSlashCommand();
}
...
export function installClaudeIntegration(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    permissions.allow = [...kept, ...slashPermissionRules()];
